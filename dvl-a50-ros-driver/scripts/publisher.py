...
def publisher():
	...
	dvl_filter_pub = rospy.Publisher('/bluerov/measurement/velocity', Measurement, queue_size=1)
	...
	while not rospy.is_shutdown():
		...
		if theDVL.velocity_valid == True:
			meas = Measurement()
			meas.header.stamp = rospy.Time.now()
			meas.header.frame_id = "dvl_bt"
			meas.value = [data["vx"], data["vy"], data["vz"]]
			meas.noise = [0.0225, 0.0225, 0.0225]
			dvl_filter_pub.publish(meas)
...
if __name__ == '__main__':
	...
	rospy.init_node('a50_pub')
	TCP_IP = rospy.get_param("~ip", "192.168.2.93")
