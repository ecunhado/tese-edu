...
public:
  ...
  */
  ...
  /**
   * Time out for no data arriving
   */
  double no_data_time;

private:
  void no_data_timeout(const boost::system::error_code& error);
...
protected:
   ...
   */
  void
  setReadCallback(const boost::function<void(const char*, size_t)>& callback);
